      ...
      <div class="row no-r-margin full-height">
        ...
        <div class="col background-white pa-1 full-height pt-4">
          <div class="container">
            ...
            <div class="apt-specification d-flex justify-content-center py-3">
              <div class="row">
                <ul class="inline">
                  <li
                    class="d-block d-md-inline"
                  >{{ (apartment.view == 'vista-ciudad-reflex') ? "Con vista a la ciudad": "Con vista al parque"}}</li>
                  ...
                  <li class="d-block d-md-inline">Valor: {{ (apartment.size == '55 M2') ? "desde $325.000.000" : (apartment.size == '65 M2') ? "$381.000.000" : "$0" }}</li>
                </ul>
              </div>
            </div>
            ...
            <NavArrows
              :canContinue="false"
              prevUrl="/personality-city-reflex"
              nextUrl="#"
            />
          </div>
        </div>
...
import registerServiceFlex from  "../../../src/services/pimex/reflexServices";
...
export default {
  ...
  methods: {
    ...
    async submitForm(e) {
      ...
      //Guardar lead
      registerServiceFlex.sendLead({
        project: "48264",
        name: this.dataUser.name,
        phone: "+57" + this.dataUser.phone,
        email: this.dataUser.email,
        origin: 'reservas.proyectoflexliving.com',
        _state: 'lead',
        referrer: 'reservas.proyectoflexliving.com',
        custom: {
    ...
    checkAvailable() {
      return new Promise((resolve, reject) => {
        // ask for data
        let reservedPr = registerServiceFlex.allLeadsReserved();
        let wonPr = registerServiceFlex.allLeadsWon();
        let allData = [];
        ...
        Promise.all([reservedPr, wonPr]).then((values) => {
          ...
          allData = allData.filter(lead => lead.custom.view && lead.custom.view === 'vista-ciudad-reflex');
